...
int	load_texture(t_game *game)
{
	...
	if (game->wall == NULL)//message derreur
		return(0);
	game->wall_frame2 = mlx_load_png("asset/textures/after/64/black_hole_frame2.png");
	if (game->wall_frame2 == NULL)//message derruer
		return(0);
	game->wall_frame3 = mlx_load_png("asset/textures/after/64/black_hole_frame3.png");
	if (game->wall_frame3 == NULL)//message derreur
		return(0);
...
void	set_depth(t_game *game, char w, int x, int y)
{
	...
	else if (w == '1')
	{
		...
		mlx_image_to_window(game->mlx, game->image_wall_frame2, x * TILE_SIZE, y * TILE_SIZE);
		mlx_image_to_window(game->mlx, game->image_wall_frame3, x * TILE_SIZE, y * TILE_SIZE);
		mlx_set_instance_depth(&game->image_wall->instances[game->image_wall->count - 1], WALL);
		mlx_set_instance_depth(&game->image_wall->instances[game->image_wall_frame2->count - 1], WALL -2);
		mlx_set_instance_depth(&game->image_wall->instances[game->image_wall_frame3->count - 1], WALL -2);
	}
	else if (w == 'C')
	{
